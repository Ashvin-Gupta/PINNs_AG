...
class PINN():
    ...
    def __init__(self, dynamics, dim, heter, inverse):
        ...
        self.epochs_init = 5000 # number of epochs for training initial phase
        self.epochs_main = 10000 # number of epochs for main training phase
        self.lr = 0.0001 # learning rate
        self.saved_path = 'data/run1/-50019.ckpt'
    ...
    def define_pinn(self, geomtime, input_data, observe_train):
        ...
        self.model.restore(self.saved_path)
        return 0
    ...
    def train(self, out_path, params):
        ...
        if self.dim ==1:
            ...
            print("This is the loss history")
            print("------------------------")
            print(type(losshistory))
            print("------------------------")
        elif self.dim == 2:
            losshistory, train_state = self.train_load(out_path, params)
    ...
    def train_1_phase(self, out_path, params):
        if self.inverse:
            ...
            #checker = dde.callbacks.ModelCheckpoint("model/model.ckpt", save_better_only = True, period =200)
            losshistory, train_state = self.model.train(epochs=self.epochs_main, model_save_path = out_path, callbacks=[checker])
        else:
            #checker = dde.callbacks.ModelCheckpoint("model/model.ckpt", save_better_only = True, period =200)
            checker = dde.callbacks.EarlyStopping(0.000001, 10, baseline = None, monitor = 'loss_train')
            losshistory, train_state = self.model.train(epochs=self.epochs_main, model_save_path = out_path, model_restore_path = "./NoTraining/20000-20001.ckpt" , callbacks = [checker])
        return losshistory, train_state
    
    def train_load(self, out_path, params):
        self.model.compile("adam", lr=self.lr)
        losshistory, train_state = self.model.train(epochs=self.epochs_main, model_save_path = out_path,model_restore_path = self.saved_path)
        return losshistory,train_state
    ...
    def train_3_phase(self, out_path, params):
        ...
        if self.inverse:
            ...
            ## Main phase
            self.model.compile("adam", lr=self.lr)
            losshistory, train_state = self.model.train(epochs=self.epochs_main, model_save_path = out_path,model_restore_path = 'data/test-10001.ckpt')
            ## Final phase
            self.model.compile("L-BFGS-B")
