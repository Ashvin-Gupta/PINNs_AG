...
class PINN():
    ...
    def __init__(self, dynamics, dim, heter, inverse):
        ...
        self.MAX_MODEL_INIT = 32# maximum number of times allowed to initialize the model
        self.MAX_LOSS = 100 # upper limit to the initialized loss
    ...
    def modify_3d_const(self):
        ## Update the PINN design for 3D and/or heterogeneity geometry
        if self.dim == 3:
            self.input = 4
    ...
    def define_pinn(self, geomtime, input_data, observe_train):
        ...
        elif self.dim == 3 and self.heter:
            if self.inverse and 'd' in self.inverse:
                pde = self.dynamics.pde_3D_heter
                self.net.apply_output_transform(self.dynamics.modify_inv_heter)
            else:
                pde = self.dynamics.pde_3D_heter_forward
                self.net.apply_output_transform(self.dynamics.modify_heter)
        elif self.dim == 3 and not self.heter:
            pde = self.dynamics.pde_3D
    ...
    def stable_init(self):
        ...
        loss_weights = [1,0.2,3.8,0.2,0.2]
        losshistory, _ = self.model.train(epochs=2)
        initial_loss = max(losshistory.loss_train[0])
        num_init = 1
        while initial_loss>self.MAX_LOSS or np.isnan(initial_loss):
            num_init += 1
            self.model = dde.Model(self.pde_data, self.net)
            self.model.compile("adam", lr=self.lr, loss_weights=loss_weights)
