...
class system_dynamics():
    ...
    def generate_data(self, file_name, dim):
        ...
        if dim == 1:
            ...
            raise ValueError('Dimesion value argument has to be either 1, 2 or 3')
        self.max_t = np.max(t)
        self.max_x = np.max(x)
        X = X.reshape(-1, 1)
        T = T.reshape(-1, 1)
    ...
    def geometry_time(self, dim):
        if dim == 1:
            ...
            geom = dde.geometry.Cuboid([self.min_x,self.min_y, self.min_z], [self.max_x,self.max_y, self.max_z])
            timedomain = dde.geometry.TimeDomain(self.min_t, self.max_t)
            geomtime = dde.geometry.GeometryXTime(geom, timedomain)
        else:
            raise ValueError('Dimesion value argument has to be either 1, 2 or 3')
...
#        dv_dt = dde.grad.jacobian(V, x, i=0, j=3)
#        dv_dxx = dde.grad.hessian(V, x, i=0, j=0)
#        dv_dyy = dde.grad.hessian(V, x, i=1, j=1)
#        dv_dzz = dde.grad.hessian(V, x, i=2, j=2)
#        dw_dt = dde.grad.jacobian(W, x, i=0, j=3)
        ## Coupled PDE+ODE Equations
    ...
    def BC_func(self,dim, geomtime):
        if dim == 1:
            ...
            bc = dde.NeumannBC(geomtime, lambda x:  np.zeros((len(x), 1)), self.boundary_func_3d_cube, component=0)
        return bc
