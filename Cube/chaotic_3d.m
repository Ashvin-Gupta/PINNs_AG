...
crossfgeo=false(X,Y,Z); % extra stimulus to generate spiral wave
crossfgeo(:,1:floor(X/2.5),:)=true;

crossfgeo2=false(X,Y,Z); % extra stimulus to generate spiral wave
crossfgeo2(20:40,20:40,:)=true;

crossfgeo3=false(X,Y,Z); % extra stimulus to generate spiral wave
...
Vsav=zeros(ncells_x,ncells_y,thick,ceil((tend-tstar)/gathert)); % array where V will be saved during simulation
Wsav=zeros(ncells_x,ncells_y,thick,ceil((tend-tstar)/gathert)); % array where W will be saved during simulation


ind=0; %iterations counter

yfun=zeros(2,size(V,1),size(V,2),size(V,3));
...
%             Istim=Ia*crossfgeo3;
        ...
        if t>=tstar&&mod(ind,gathert)==0
            % save values
            Vsav(:,:,:,index)=V(2:end-1,2:end-1,2:end-1);
            Wsav(:,:,:,index)=W(2:end-1,2:end-1,2:end-1);
            index = index +1;
            % show (thicker) cable
            if flagmovie
                subplot(2,1,1)
