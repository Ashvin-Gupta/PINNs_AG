...
Vsav=zeros(ncells,ncells,thick,ceil((tend-tstar)/gathert)); % array where V will be saved during simulation
Wsav=zeros(ncells,ncells,thick,ceil((tend-tstar)/gathert)); % array where W will be saved during simulation
...
Vsav = permute(Vsav, [4,1,2,3]);
Wsav = permute(Wsav, [4,1,2,3]);
%save([matname '.mat'],'Vsav','Wsav','x','t','y','z','-v7')
...
for t = 1:timePoints
    ...
    % Reshape the data to 3D (remove singleton dimensions)
    data = squeeze(data);
